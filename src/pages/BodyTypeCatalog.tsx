...
const BodyTypeCatalog = () => {
  ...
  const formattedBodyType = React.useMemo(() => {
    if (!bodyType) return '';
    
    // Format like "sedan" to "Седан" or "suv" to "Внедорожник"
  ...
  useEffect(() => {
    ...
    // Set body type filter when component mounts or bodyType changes
    const newFilter = { ...filter, bodyType, bodyTypes: [bodyType] };
    setFilter(newFilter);
  ...
  // Check if body type exists in our database
  const bodyTypeExists = React.useMemo(() => {
    const bodyTypes = [...new Set(cars.map(car => car.bodyType))];
    return bodyTypes.some(bt => bt.toLowerCase() === bodyType?.toLowerCase());
  }, [cars, bodyType]);

  // If body type doesn't exist, redirect to main catalog
  useEffect(() => {
    if (!loading && cars.length > 0 && !bodyTypeExists && bodyType) {
      navigate('/cars', { replace: true });
    }
  }, [loading, cars, bodyTypeExists, bodyType, navigate]);
      ...
      <main className="flex-grow">
        <section className="py-6 md:py-10 bg-auto-gray-50">
          <div className="container mx-auto px-4">
            <h1 className="text-3xl font-bold mb-6">Автомобили типа {formattedBodyType}</h1>
...
export default BodyTypeCatalog;
